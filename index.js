...
function generateDetailedResultData (resultData, subIndicators, investments, charts) {
  return resultData.map(geo => {
    let timeSeriesCharts = charts
      .filter(c => c.type === 'timeSeries')
      .map(c => generateTimeSeries(geo, subIndicators, c))

    let energyInvestmentChart = generateEnergyInvestment(geo, investments)

    return {
      ...geo,
      charts: [].concat(timeSeriesCharts, energyInvestmentChart)
    }
  })
}
...
function generateTimeSeries (geo, data, chart) {
  return {
    ...
    'data': data
      .filter(i => i.id === chart.indicator_id && i.geography === geo.name)
      .map(i => ({
        name: i.subindicator,
        values: i.values
      }))
...
(async function main () {
  try {
    ...
    const detailedResultData = generateDetailedResultData(resultData, subIndicators, investments, charts)
