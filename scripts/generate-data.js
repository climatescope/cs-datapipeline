...
function generateDetailedResultData (resultData, indicators, charts, conf) {
  return resultData.map(geo => {
    ...
    return {
      ...
      sectionCopy: generateSectionCopy(geo, indicators, conf.subindicators.maxYear)
    }
  })
}
...
function generateSectionCopy (geo, indicators, year) {
  let countryCopy = indicators
    ...
    .filter(i =>
      ...
      i.values.find(v => v.year === year).value
    )
  ...
  if (countryCopy.length) {
    return countryCopy.map(section => {
      ...
      return {
        'id': dashify(title),
        'name': title,
        'value': section.values.find(v => v.year === year).value
      }
    })
  } else {
